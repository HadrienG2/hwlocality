...
impl Topology {
    ...
    pub fn memory_attribute_named(&self, name: &str) -> Result<Option<MemoryAttribute>, NulError> {
        ...
        match res {
            // SAFETY: On positive return, we can assume that hwloc set id to a
            //         valid memory attribute ID.
...
impl MemoryAttributeBuilder<'_, '_> {
    ...
    ///
    /// [`BadInitiatorsCount`]: InitiatorsError::BadInitiatorsCount
    /// [`NeedInitiators`]: InitiatorsError::NeedInitiators
    /// [`UnwantedInitiators`]: InitiatorsError::UnwantedInitiators
...
pub enum InitiatorsError {
    ...
    #[error("the number of provided initiators does not match the number of attribute values")]
    BadInitiatorsCount,
}
    ...
    (
        ...
        $(
            ...
            pub const fn $constructor(topology: &'topology Topology) -> Self {
                // SAFETY: Per type invariant of MemoryAttributeID, the inner ID
                //         is indeed a valid memory attribute ID.
...
impl<'topology> MemoryAttribute<'topology> {
    ...
    /// # Safety
    ///
    /// `id` must be one of the `HWLOC_MEMATTR_ID_` constants or a new id
    ...
    /// # Errors
    ///
    /// - [`UnknownAttribute`] if no such attribute exists.
    ///
    ...
    pub fn flags(&self) -> Result<MemoryAttributeFlags, MemoryAttributeQueryError> {
    ...
    /// # Errors
    ///
    /// - [`NoInitiator`] if this attribute does not relate to a specific initiator
    /// - [`UnknownAttribute`] if no such attribute exists.
    ///
    ...
    fn checked_initiator(
        ...
        is_optional: bool,
    ) -> Result<hwloc_location, MemoryAttributeQueryError> {
        ...
        is_good
            .then(|| initiator.map_or(NULL_LOCATION, MemoryAttributeLocation::into_raw))
...
pub enum MemoryAttributeQueryError {
    /// No such memory attribute exists
    #[error("no such memory attribute exists")]
    UnknownAttribute,

    /// Incorrect `initiator` parameter
    ///
    /// Either an initiator had to be specified and was not specified, or the
    /// requested attribute has no notion of initiator but an initiator
    /// was specified nonetheless.
    #[error("specified initiator is incorrect for this memory attribute")]
    BadInitiator,

    /// Asked for an initiator, but this memory attribute doesn't have one
    ///
    /// This error occurs when querying initiators on attributes that do not
    /// have initiators (like NUMA node capacity).
...
impl<'target> MemoryAttributeLocation<'target> {
    ...
    pub(crate) fn into_raw(self) -> hwloc_location {
        match self {
            Self::CpuSet(cpuset) => hwloc_location {
                ty: HWLOC_LOCATION_TYPE_CPUSET,
                location: hwloc_location_u {
                    cpuset: cpuset.as_ptr(),
                },
            },
            Self::Object(object) => hwloc_location {
    ...
    /// # Safety
    ///
    /// Only use this on `hwloc_location` structs from hwloc topology queries
    unsafe fn from_raw(
        _topology: &'target Topology,
        raw: hwloc_location,
    ) -> Result<Self, UnknownLocationType> {
        match raw.ty {
            HWLOC_LOCATION_TYPE_CPUSET => unsafe {
            ...
            HWLOC_LOCATION_TYPE_OBJECT => unsafe {
...
impl TargetNumaNodes<'_> {
    ...
    pub(crate) fn into_raw_params(self) -> (hwloc_location, LocalNUMANodeFlags) {
        match self {
            Self::Local { location, flags } => (location.into_raw(), flags),
            Self::All => (NULL_LOCATION, LocalNUMANodeFlags::ALL),
        }
    }
}
//
